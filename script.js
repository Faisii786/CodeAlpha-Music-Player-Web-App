document.addEventListener("DOMContentLoaded", function () {
  ...
  async function getMetadata(file) {
    ...
    return {
      ...
      album: "Unknown Album",
      genre: "Unknown Genre",
      artwork: "./images/default.png", // Placeholder for album artwork
    };
  }
  ...
  function loadAndPlayTrack(index, playlistToUse = playlist) {
    ...
    currentTrackImage.src = metadata.artwork || "default.jpg";
  ...
  searchInput.addEventListener("input", function () {
    ...
    const filteredTracks = playlist.filter(
      ({ metadata }) =>
        ...
        metadata.artist.toLowerCase().includes(searchQuery) ||
        metadata.album.toLowerCase().includes(searchQuery) ||
        metadata.genre.toLowerCase().includes(searchQuery)
    );
    displayPlaylist(filteredTracks);

    // Reset current track index to 0 after search to avoid confusion
    currentTrackIndex = 0;
  });
